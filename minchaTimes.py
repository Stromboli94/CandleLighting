...
for i in range(years):
    ...
    next_rh_dow = (hdate.HebrewDate(h.hdate.year + 1, hdate.Months.TISHREI, 1)).to_gdate().weekday()
    gdate = rh.to_gdate()
    currentShabbos = hdate.HDateInfo(gdate, diaspora= diaspora).upcoming_shabbat
    weeks = h.hdate.year_size(h.hdate.year) // 7
    for j in range(weeks + (0 if next_rh_dow == 5 else 1)):
    ...
    tod = tod.replace(year = tod.year + 1)
